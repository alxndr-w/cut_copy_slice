<?php
/**
 * cut_copy_slice class - basic functions for the addon and its plugins.
 */
class cut_copy_slice extends cut_copy_slice_abstract
{
    /**
    ...
    public static function init(rex_extension_point $ep)
    {
        if (rex::isBackend() && rex::getUser()) {
            ...
            cut_copy_slice_backend::init($ep);
        } elseif (!rex::isBackend()) {
            // things to do in frontend
            rex_extension::register('SLICE_SHOW', ['cut_copy_slice', 'showSlice'], rex_extension::EARLY);
        }
    }
