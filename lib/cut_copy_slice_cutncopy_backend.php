...
class cut_copy_slice_cutncopy_backend extends cut_copy_slice_backend
{
    ...
    public static function init(rex_extension_point $ep)
    {
        if (rex::getUser()) {
            ...
            rex_extension::register('STRUCTURE_CONTENT_SLICE_MENU', ['cut_copy_slice_cutncopy_backend', 'addButtons']);

            // process any cut or copy call
            rex_extension::register('STRUCTURE_CONTENT_BEFORE_SLICES', ['cut_copy_slice_cutncopy_backend', 'process']);

            // insert the "INSERT BLOCK" link into module dropdown
            rex_extension::register('STRUCTURE_CONTENT_MODULE_SELECT', ['cut_copy_slice_cutncopy_backend', 'addBlockToDropdown']);

            // post process the clipboard after a slice is inserted into another article
            rex_extension::register('SLICE_ADDED', ['cut_copy_slice_cutncopy_backend', 'postProcessClipboard']);
        } else {
            ...
            foreach (['copy', 'cut'] as $type) {
                static::addButton($ep, [
                    ...
                    'attributes' => [
                        ...
                        'data-cut_copy_slice-cutncopy-iscopied' => $is_copied && ($action === $type) ? 'true' : 'false',
    ...
    {
        $function = rex_request('cut_copy_slice', 'string', null);
        ...
        if ($function === static::plugin()->getName()) {
            ...
            if (method_exists('cut_copy_slice_cutncopy_backend', $action)) {
                        ...
                        if ($slice && true === rex::getUser()->getComplexPerm('modules')->hasPerm($slice->getModuleId())) {
                            ...
                            if (count($article = $sql->getArray())) {
                                ...
                                $_POST = array_replace($_POST, [
                                    ...
                                    'cut_copy_slice_cutncopy_action' => $action,
                                ]);
                            }
    ...
    public static function postProcessClipboard(rex_extension_point $ep)
    {
        ...
        if (static::$clipboard_slice instanceof rex_article_slice) {
            if ((int) static::$clipboard_slice->getId() === static::getCookie('slice_id', 'int', null)) {
                $action = static::getCookie('action', 'string', '');
                if ($action === rex_post('cut_copy_slice_cutncopy_action', 'string', null)) {
        ...
        if ($slice_id && $action) {
            ...
            if ($slice) {
                ...
                if (preg_match('/href="([^"]+module_id=' . $slice->getModuleId() . '[^"]+)"/ismu', $subject, $matches)) {
                    ...
                    $subject = $prefix . '<li class="cut_copy_slice-cutncopy-clipboard-slice is--' . $action . '"><a href="' . $url . '" data-pjax="true" data-pjax-no-history="true">' . static::package()->i18n('insert_slice', static::getModuleName($slice->getModuleId()), $slice->getId(), $slice->getArticle()->getName()) . '</a></li>' . $suffix;
